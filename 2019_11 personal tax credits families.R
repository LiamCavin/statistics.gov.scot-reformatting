...
colnames_lafamilies <- c("GeographyCode", "", "", "", "", "", "AllTaxCredit", "",
                       "InWorkWTCAndCTC", "CTC", "WTC", "TotalInWork", "InWorkChildcare",
                       "", "", "TotalLoneParent", "LoneParentChildcare", 
                       "", "AllOutOfWork", "OutOfWorkLoneParent", "OutOfWorkCouples")

colnames_dzfamilies <- c("", "", "GeographyCode", "", "", "", "AllTaxCredit", "",
                         "InWorkWTCAndCTC", "CTC", "WTC", "TotalInWork", "InWorkChildcare",
                         "", "", "TotalLoneParent", "LoneParentChildcare", 
                         "", "AllOutOfWork", "OutOfWorkLoneParent", "OutOfWorkCouples")
...
keepcols <- c("GeographyCode", "AllTaxCredit", "InWorkWTCAndCTC", 
              "CTC", "WTC", "TotalInWork", "InWorkChildcare",
              "TotalLoneParent", "LoneParentChildcare", 
              "AllOutOfWork", "OutOfWorkLoneParent", "OutOfWorkCouples")

# get la data, drop junk columns, and replace "-" with 0
la_families <- data.frame(read_xlsx("LA1718.xlsx", sheet = 1, range = "B475:V508", 
                             col_names = colnames_lafamilies))
la_families <- la_families[-2, keepcols]
la_families[la_families=="-"] <- 0

# get dz data, drop junk columns , and replace "-" with 0
dz_families <- data.frame(read_xlsx("DZ1718.xlsx", sheet = 1, range = "c10:w6986", 
                                        col_names = colnames_dzfamilies))
dz_families <- dz_families[, keepcols]
dz_families[dz_families=="-"] <- 0

# make all columns numeric
for (i in 2:12) {
  la_families[,i] <- as.numeric(la_families[,i]) 
}
sapply(dz_families, class)

for (i in 2:12){
  dz_families[,i] <- as.numeric(dz_families[,i]) 
}
...
laf_tidy <- pipe.format(la_families, 2, "2017/18", 1000)
for (i in 3:12){
  laf_tidy <- rbind(laf_tidy, pipe.format(la_families, i, "2017/18", 1000))
}

dzf_tidy <- pipe.format(dz_families, 2, "2017/18", 1)
for (i in 3:12){
  dzf_tidy <- rbind(dzf_tidy, pipe.format(dz_families, i, "2017/18", 1))
}

laf_tidy <- reformat.tax.credits(laf_tidy)
dzf_tidy <- reformat.tax.credits(dzf_tidy)

families_forupload <- rbind(laf_tidy, dzf_tidy)

# Lastly, the HMRC data includes some obsolete geography codes. Replace them.
families_forupload[,1] <-  str_replace_all(families_forupload[,1], fixed("S12000015"), "S12000047")
families_forupload[,1] <-  str_replace_all(families_forupload[,1], fixed("S12000024"), "S12000048")
families_forupload[,1] <-  str_replace_all(families_forupload[,1], fixed("S12000044"), "S12000050")
families_forupload[,1] <-  str_replace_all(families_forupload[,1], fixed("S12000046"), "S12000049")

write.csv(families_forupload, "families1718_forupload.csv", row.names=FALSE)
